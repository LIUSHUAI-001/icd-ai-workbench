...
## 43. SHIFT 多线平移到目标节点失效修复（phantom pointerEvents 拦截 mouseup）

### 43.1 用户报告

> 多条连线按住 SHIFT 平移到其他节点失效了，连不上去。从一个 Seedance2.0 包含图像/音频/视频的多种连线，平移到另一个 Seedance2.0，没有反应。

### 43.2 SHIFT 批量移线机制回顾

[Canvas.tsx 1520-1748 行](file:///e:/PenguinPravite/T8-penguin-canvas/src/components/Canvas.tsx#L1520-L1748) 在全局 `mousedown` capture 阶段拦截 SHIFT + handle 左键按下：

1. 收集起点节点的所有入/出边（按 handleType）
2. 创建一个隐形 `bulkPhantom` 节点（id = `__bulk_phantom__`，两个 opacity:0 width:1 height:1 的 Handle）
3. 把所有相关边的 `target` 或 `source` 重定向到 phantom
4. `mousemove` 实时把 phantom 位置更新为 `screenToFlowPosition({clientX, clientY})` → 边跟随鼠标移动
5. `mouseup` 检测落点 handle，执行批量重连

### 43.3 根因

**phantom 节点的最外层 wrapper DOM 在 `mouseup` 瞬间盖在目标节点的 handle 之上**。

- ReactFlow 给每个 node 渲染的 `.react-flow__node` wrapper 默认接收所有指针事件
- phantom 节点位置紧跟鼠标 → wrapper 始终在光标正下方
- `mouseup.event.target` 因此命中 phantom 的 DOM，而不是用户视觉上瞄准的目标 handle
- `target.closest('.react-flow__handle')` 返回 null（或仅命中 phantom 自己的隐形 handle）
- 直接走 `restoreOriginal()` → 边还原到起点节点 → **平移看似没反应**

mousemove 中虽然用了 `document.elementFromPoint`，但它只返回最顶层元素，仍会被 phantom 拦截 → hover handle 高亮也不可靠。
...
setNodes((ns) => [...ns, {
  ...
  deletable: false,
  zIndex: 9999,
  style: { pointerEvents: 'none' },   // ← 关键：让鼠标穿透 phantom
} as Node]);
```

效果：phantom wrapper 不再吃鼠标事件，`mouseup.target` / `elementFromPoint` 直接落到下层真实 handle。

#### B. 新增 `findHandleAt(cx, cy)` 工具，多重兜底

```ts
const findHandleAt = (cx: number, cy: number): HTMLElement | null => {
  const els = document.elementsFromPoint(cx, cy);   // 注意是 elements (复数)
  for (const el of els) {
    ...
    if (nid === BULK_PHANTOM_ID) continue;          // 跳过 phantom 自身
    return h;
  }
  return null;
};
...
`elementsFromPoint`（复数）返回该坐标下**所有**命中元素（按 z 顺序），可遍历跳过 phantom 找到真正的 handle。这是兜底层防御，万一 A 步骤被某些边界情况绕过仍能命中。

#### C. `onMouseUp` 双层路径

```ts
const onMouseUp = (upEv: MouseEvent) => {
  // 快路径：event.target 直接 closest
  const upTargetEl = upEv.target as HTMLElement | null;
  let upHandleEl = upTargetEl?.closest('.react-flow__handle') as HTMLElement | null;

  // 校验：若命中的是 phantom 自身 handle 视为未命中
  if (upHandleEl) {
  ...
  // ... 后续校验 handleType / nodeId 不变
};
```

#### D. `onMouseMove` 同步用 `findHandleAt` 高亮

```ts
const hoverHandle = findHandleAt(mv.clientX, mv.clientY);
if (hoverHandle) {
  // 排除起点节点 + handleType 校验 → setHoverHL
}
```
...
| 决策 | 原因 |
|------|------|
| phantom `pointerEvents:'none'`（A）+ `elementsFromPoint` 穿透（B）双保险 | A 解决 95% 场景；B 防 React 异步渲染 / 某些浏览器 hit-test 时序边界 |
| 仅给 phantom 加 `pointerEvents:'none'`，不动其他节点 | phantom 是临时节点，禁用事件不影响业务；其他节点必须能正常接收 mouseup 完成连接 |
| `findHandleAt` 跳过 `BULK_PHANTOM_ID` 而不是按 className | phantom 的 handle 是无样式 invisible，未来若改造件易漏掉 className；按 nodeId 永远稳定 |
| 保留快路径优先 | event.target 直接命中是最常见路径，性能更好；穿透查找仅作兜底 |

### 43.6 兼容性

- 普通连线（非 SHIFT）：未走 phantom 分支，零影响
- SHIFT 单边断连 / hover 高亮：路径相同，hover 反馈更准
- ESC 取消批量移线：`restoreOriginal` 流程不变
- 端口染色 / handleType 校验：保留
- 与剪刀模式（§39）共存：剪刀走 `.react-flow__pane` / GroupBox 空白判断，批量移线走 `.react-flow__handle` 命中，互不冲突

### 43.7 验收清单

- [ ] 从 SeedanceNode 入口（target）SHIFT+拖出 → 拖到另一个 SeedanceNode 入口释放 → 所有入边批量改连
- [ ] 同上场景但拖出后释放在画布空白处 → 边自动还原到原起点（restoreOriginal）
- [ ] 拖出过程中 hover 到目标节点 handle → 该 handle 出现绿色光晕高亮
- [ ] ESC 中途取消 → 边还原
- [ ] 不同端口类型自动染色仍然正确
- [ ] 普通拖线 / 剪刀切边 / 单选 / 框选 / 节点拖动 / GroupBox 拖动联动 全部不受影响

### 43.8 关键文件

- [src/components/Canvas.tsx](file:///e:/PenguinPravite/T8-penguin-canvas/src/components/Canvas.tsx) 行 1568-1748：phantom 创建 + `findHandleAt` + `onMouseMove` + `onMouseUp`
